// Package cli assembles the pplx command tree from the perplexity
...
// NewApp assembles the kit application from the perplexity domain. The
...
//
// To add a command, declare it in perplexity/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := perplexity.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(perplexity.Domain{}).Register(app)
