...
describes what users get. Pick a format with `-o`, or let pplx choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
pplx <command> -o table   # aligned columns for reading
pplx <command> -o jsonl   # one JSON object per line, for piping
pplx <command> -o json    # a single JSON array
pplx <command> -o csv     # spreadsheet friendly
pplx <command> -o tsv     # tab-separated
pplx <command> -o url     # just the URL column
pplx <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
pplx <command> --fields id,title,url
```
...
```bash
pplx <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
pplx <command>            # a table, because this is a terminal
pplx <command> | wc -l    # JSONL, because this is a pipe
```
